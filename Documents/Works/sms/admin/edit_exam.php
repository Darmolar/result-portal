...
include('header.php');
 ...
 $exam=$obj->Get_All_Data("select * from examslist where id=".base64_decode($_GET['exam'])."");
?>
<script>
...
<div class="col-xl-12">
        ...
        <div class="form-group has-error" ng-class="{'has-error': addExam.examTitle.$invalid}">
          ...
          <div class="col-sm-10">
            <input type="text" name="examTitle" ng-model="form.examTitle" class="form-control ng-pristine ng-invalid ng-invalid-required" required="" placeholder="Exam Name" value="<?php echo $exam[0]['examTitle']  ?>" required>
          </div>
		  <br/>
		  <br/>
        </div>
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <textarea name="examDescription" class="form-control" ng-model="form.examDescription" placeholder="Exam Description" required><?php echo $exam[0]['examDescription']  ?></textarea>
          </div>
		 <br/>
		  <br/>
        </div>
          ...
          <div class="col-sm-10">
              <select class="form-control" name="examClasses[]" multiple="" required>
				 <?php
									 ...
									 ?>
									<option value="<?php echo $teacher[$i]['cl_id'] ?>" <?php if($exam[0]['class']==$teacher[$i]['cl_id']){ echo 'selected'; } ?>><?php echo $teacher[$i]['clname'] ?></option>
        ...
        <div class="form-group">
		  ...
		  <label for="fieldUser" >Sheet Field</label>
		   <div id="highschool">
					<?php
					$json=json_decode($exam[0]['field']);
for($i=0;$i<count($json);$i++){
	echo '<div><div class="col-xs-12"> <label for="fieldUser" >Field(s)</label><input type="text" name="sheet[]" value="'.$json[$i].'" id="fieldUser"  class="form-control" readonly/></div><br/><a href="#" class="remove_field">Remove</a></div>';
}
