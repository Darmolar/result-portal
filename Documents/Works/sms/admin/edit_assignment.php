...
if(isset($_GET['ass_id'])){
	$id=base64_decode($_GET['ass_id']);
	$data=$obj->Get_All_Data("select * from assignments where ass_id=$id");
}
//print_r($data);
...
<div class="col-xl-12">
        ...
        <div class="form-group has-error" ng-class="{'has-error': addDorm.AssignTitle.$invalid}">
          ...
          <div class="col-sm-10">
            <input type="hidden" name="id" ng-model="form.AssignTitle" class="form-control" required="" value="<?php echo $data[0]['ass_id'] ?>" placeholder="Assignment title">
			
            <input type="text" name="title" ng-model="form.AssignTitle" class="form-control" required="" value="<?php echo $data[0]['AssignTitle'] ?>" placeholder="Assignment title">
          </div>
        </div>
		<br/><br/>
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <textarea name="desc" class="form-control"  placeholder="Assignment Description"><?php echo $data[0]['AssignDescription'] ?></textarea>
          </div>
        </div>
			<br/><br/>
        <div class="form-group has-error">
          ...
          <div class="col-sm-10">
            <input date-picker="" type="date" id="datemask" name="deadLine" required="" class="form-control" value="<?php echo $data[0]['AssignDeadLine'] ?>">
          </div>
        </div>
			<br/><br/>
        <div class="form-group">
        ...
        <div class="form-group has-error">
          ...
          <div class="col-sm-10">
			...
			<option value="<?php echo $data[0]['classId'] ?>" >Select Class</option>
			<?php
			$class=$obj->Get_All_Data("select * from class");
			$json=json_decode($data[0]['classId']);
			//print_r($json);
			for($i=0;$i<count($class);$i++){
				
				if(isset($json[$i]) and $json[$i]==$class[$i]['cl_id']){
					$selected="selected";
				}
			?>
			<option value="<?php echo $class[$i]['cl_id']; ?>" <?php  echo $selected; ?>><?php echo $class[$i]['clname'] ?></option>
        ...
        <div class="form-group has-error" ng-class="{'has-error': addDorm['sectionId[]'].$invalid}" ng-show="$root.dashboardData.enableSections == '1'">
          ...
          <div class="col-sm-10">
			...
			$json=json_decode($data[0]['sectionId']);
			print_r($json);
			for($i=0;$i<count($section);$i++){
				
					
				if(isset($json[$i]) and $json[$i]==$section[$i]['sec_id']){
					$selected="selected";
				}
				
			?>
			<option value="<?php echo $section[$i]['sec_id'] ?>" <?php echo $selected;  ?> ><?php echo $section[$i]['sec_name'] ?></option>
        ...
        <div class="form-group has-error" ng-class="{'has-error': addDorm.subjectId.$invalid}">
          ...
          <div class="col-sm-10">
			...
			for($i=0;$i<count($sub);$i++){
				if(data[0]['subjectId']==$sub[$i]['sub_id'] ){
					$selected="selected";
				}
			?>
			<option value="<?php echo $sub[$i]['sub_id'] ?>" <?php echo $selected;  ?>><?php echo $sub[$i]['subject_name'] ?></option>
        ...
        <div class="form-group">
          <div class="col-sm-12">
           <input type="hidden" name="edit_assignment">
...
<script type="text/javascript">
						...
						jQuery("#form_data").submit(function(e){
								...
								$.ajax({
									...
									if(response ==='Assignment Updated Successfully')
									{
